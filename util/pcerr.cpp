...
namespace pcerr {
    static std::unordered_map<int, std::string> defaultMsg = {
            ...
            {LIBPERF_ERR_INVALID_BDF_VALUE, "invalid PCI BDF value"},
            {LIBPERF_ERR_NOT_SUPPORT_PCIE_BDF, "PCIe BDF is not managed by a PMU device"},
            {LIBPERF_ERR_NOT_SUPPORT_SMMU_BDF, "SMMU BDF is not managed by a PMU device"},
            {LIBPERF_ERR_NOT_SUPPORT_PCIE_COUNTING, "PCIe PMU device is unavailable"},
            {LIBPERF_ERR_NOT_SUPPORT_METRIC, "PMU metric is not supported on this system"},
            {LIBPERF_ERR_PMU_DATA_NO_FOUND, "no PMU data matched the requested device metric"},
            {LIBPERF_ERR_NOT_SUPPORT_PCIE_PORT, "PCIe port is not managed by a PMU device"},
